...
def greyscale_multilevel_uniform(
    ...
    for level_idx in range(count_max_levels):
        ...
        for mh_idx in range(count_mh_steps):
            # Propose new sample
            g_bottom = dist.Uniform(low=torch.max(x - width_proposal.view(-1,1,1,1)*(x_max-x_min), prior.low),
                                    high=torch.min(x + width_proposal.view(-1,1,1,1)*(x_max-x_min), prior.high),validate_args=False)
            ...
            g_top = dist.Uniform(low=torch.max(x_maybe - width_proposal.view(-1,1,1,1)*(x_max-x_min), prior.low),
                                 high=torch.min(x_maybe + width_proposal.view(-1,1,1,1)*(x_max-x_min), prior.high),validate_args=False)
...
def multilevel_uniform(
    ...
    for level_idx in range(count_max_levels):
        ...
        for mh_idx in range(count_mh_steps):
            # Propose new sample
            g_bottom = dist.Uniform(low=torch.max(x - width_proposal.view(-1,1,1,1)*(x_max-x_min).view(3,1,1), prior.low),
                                    high=torch.min(x + width_proposal.view(-1,1,1,1)*(x_max-x_min).view(3,1,1), prior.high),validate_args=False)
            # g_bottom = dist.Normal(x, width_proposal.unsqueeze(-1))
            ...
            # Calculate log-acceptance ratio
            g_top = dist.Uniform(low=torch.max(x_maybe - width_proposal.view(-1,1,1,1)*(x_max-x_min).view(3,1,1), prior.low),
                                 high=torch.min(x_maybe + width_proposal.view(-1,1,1,1)*(x_max-x_min).view(3,1,1), prior.high),validate_args=False)
            # g_top = dist.Normal(x_maybe, width_proposal.unsqueeze(-1))
